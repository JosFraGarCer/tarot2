// server/api/arcana/index.get.ts
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // DB index suggestion:
    // - idx_arcana_code (code)
    // - idx_arcana_status (status)
    // - idx_arcana_translations_fk_lang (arcana_id, language_code)
    // - idx_arcana_translations_lang (language_code)
    ...
    if (is_active !== undefined) base = base.where('a.is_active', '=', is_active)
    if (created_by !== undefined) base = base.where('a.created_by', '=', created_by)

    // Tag filters (AND semantics)
    if (tagIdsArray && tagIdsArray.length > 0) {
      base = base.where(sql`a.id in (
        select tl.entity_id
        from tag_links tl
        where tl.entity_type = ${'arcana'} and tl.tag_id = any(${tagIdsArray})
