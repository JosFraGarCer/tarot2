...
export const skillCrud = createCrudHandlers({
  ...
  eagerLoad: [
    ...
    {
      key: 'editorial_state',
      fetch: (db: any, ids: number[]) => fetchEditorialStateBatch(db, 'base_skills', ids),
      defaultValue: null,
    },
  ],
  ...
  onTranslationUpsert: async (entityId, lang, userId) => {
    ...
    if (!db) return
    await upsertTranslationState(db, 'base_skills', entityId, lang, userId)
  },
  onTranslationDelete: async (entityId, lang) => {
    const db = globalThis.db
    if (!db) return
    await deleteTranslationState(db, 'base_skills', entityId, lang)
  },
  onBaseDelete: async (entityId) => {
    const db = globalThis.db
    if (!db) return
    await deleteAllTranslationStates(db, 'base_skills', entityId)
    await deleteEditorialState(db, 'base_skills', entityId)
  },
  logScope: 'skill',
})
