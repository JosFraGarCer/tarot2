// server/api/base_skills/index.get.ts
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const { page, pageSize, q: search, status, is_active, created_by, facet_id } = safeParseOrThrow(querySchema, q)
    const lang = getRequestedLanguage(q)

    // DB index suggestion:
    // - idx_base_skills_code (code)
    // - idx_base_skills_status (status)
    // - idx_base_skills_translations_fk_lang (base_skill_id, language_code)
    // - idx_base_skills_translations_lang (language_code)
    ...
    if (is_active !== undefined) base = base.where('s.is_active', '=', is_active)
    if (created_by !== undefined) base = base.where('s.created_by', '=', created_by)
    if (facet_id) base = base.where('s.facet_id', '=', facet_id)

    // Tag filters (AND semantics)
    if (tagIdsArray && tagIdsArray.length > 0) {
      base = base.where(sql`s.id in (
        select tl.entity_id
        from tag_links tl
        where tl.entity_type = ${'skills'} and tl.tag_id = any(${tagIdsArray})
    ...
    globalThis.logger?.info('Base skills listed', {
      page: p,
      pageSize: ps,
      count: data.length,
    ...
    globalThis.logger?.error('Failed to fetch base skills', {
