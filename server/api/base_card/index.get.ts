// server/api/base_card/index.get.ts
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // DB index suggestion:
    // - idx_base_card_code (code)
    // - idx_base_card_status (status)
    // - idx_base_card_translations_fk_lang (card_id, language_code)
    // - idx_base_card_translations_lang (language_code)
    ...
    if (is_active !== undefined) base = base.where('c.is_active', '=', is_active)
    ...
    if (tagIdsArray && tagIdsArray.length > 0) {
      base = base.where(sql`c.id in (
        select tl.entity_id
        from tag_links tl
        where tl.entity_type = ${'base_card'} and tl.tag_id = any(${tagIdsArray})
    ...
    globalThis.logger?.info('Base cards listed', {
      ...
      card_type_id: card_type_id ?? null,
