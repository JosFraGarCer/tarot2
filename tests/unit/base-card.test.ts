...
describe('BaseCard Unit Tests', () => {
  ...
  describe('Create Test Entity', () => {
    it('should create base-card "Test" with multiple fields', async () => {
      const mockResponse = {
        ...
        data: {
          ...
          data: {
            ...
            description: 'A test base-card for unit testing',
            status: 'draft',
            is_active: true,
      ...
      mockGetEntityEndpoint.mockReturnValue('/api/base_card')
  ...
  describe('Modify Test Entity Fields', () => {
    ...
    beforeEach(async () => {
      ...
      const mockGetEntityEndpoint = getEntityEndpoint as MockedFunction<typeof getEntityEndpoint>
      mockGetEntityEndpoint.mockReturnValue(`/api/base_card/${createdEntityId}`)
    })
    ...
    it('should modify name field', async () => {
      ...
      const updateData = { name: 'Updated Test BaseCard Name' }
    ...
    it('should modify description field', async () => {
      ...
      const updateData = { description: 'Updated description for test base-card' }
      const response = await authenticatedApi.patch(getEntityEndpoint(entityType, createdEntityId), updateData)
  ...
  describe('Create Fields in Spanish (es)', () => {
    it('should create base-card with Spanish fields', async () => {
      const { authenticatedApi, createTestData, getEntityEndpoint } = await import('../integration/helpers')
      const mockCreateTestData = createTestData as MockedFunction<typeof createTestData>
      const mockGetEntityEndpoint = getEntityEndpoint as MockedFunction<typeof getEntityEndpoint>
      const mockPost = authenticatedApi.post as MockedFunction<typeof authenticatedApi.post>
      ...
      const mockResponse = {
        ...
        data: {
          ...
          data: {
            ...
            description: 'Una carta base de prueba para pruebas unitarias',
            lang: 'es',
            status: 'draft',
            is_active: true,
          }
      ...
      expect(response.status).toBe(201)
      expect(response.data?.success).toBe(true)
      expect(response.data?.data.name).toBe('Carta Base de Prueba')
