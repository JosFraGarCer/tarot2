...
describe('CardType Unit Tests', () => {
  const entityType = 'cardtype'
  const testCode = 'test-cardtype'
  ...
  describe('Create Test Entity', () => {
    it('should create cardtype "Test" with multiple fields', async () => {
      const mockResponse = {
        ...
        data: {
          ...
          data: {
            ...
            description: 'A test cardtype for unit testing',
            status: 'draft',
            is_active: true,
      ...
      mockGetEntityEndpoint.mockReturnValue('/api/card_type')
      ...
      expect(response.data?.data.description).toBe('A test cardtype for unit testing')
  ...
  describe('Modify Test Entity Fields', () => {
    ...
    beforeEach(async () => {
      ...
      const mockGetEntityEndpoint = getEntityEndpoint as MockedFunction<typeof getEntityEndpoint>
      mockGetEntityEndpoint.mockReturnValue(`/api/card_type/${createdEntityId}`)
    })
    ...
    it('should modify name field', async () => {
      ...
      const updateData = { name: 'Updated Test CardType Name' }
    ...
    it('should modify description field', async () => {
      ...
      const updateData = { description: 'Updated description for test cardtype' }
      const response = await authenticatedApi.patch(getEntityEndpoint(entityType, createdEntityId), updateData)
  ...
  describe('Create Fields in Spanish (es)', () => {
    it('should create cardtype with Spanish fields', async () => {
      const { authenticatedApi, createTestData, getEntityEndpoint } = await import('../integration/helpers')
      const mockCreateTestData = createTestData as MockedFunction<typeof createTestData>
      const mockGetEntityEndpoint = getEntityEndpoint as MockedFunction<typeof getEntityEndpoint>
      const mockPost = authenticatedApi.post as MockedFunction<typeof authenticatedApi.post>
      ...
      const mockResponse = {
        ...
        data: {
          ...
          data: {
            ...
            description: 'Un tipo de carta de prueba para pruebas unitarias',
            lang: 'es',
            status: 'draft',
            is_active: true,
          }
      ...
      expect(response.status).toBe(201)
      expect(response.data?.success).toBe(true)
      expect(response.data?.data.name).toBe('Tipo de Carta de Prueba')
      expect(response.data?.data.description).toBe('Un tipo de carta de prueba para pruebas unitarias')
