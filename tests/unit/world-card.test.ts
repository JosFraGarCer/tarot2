...
describe('WorldCard Unit Tests', () => {
  const entityType = 'world-card'
  const testCode = 'test-world-card'
  ...
  describe('Create Test Entity', () => {
    it('should create world-card "Test" with multiple fields', async () => {
      const mockResponse = {
        ok: true,
        status: 201,
        data: {
          ...
          data: {
            id: 1,
            code: testCode,
            name: 'Test WorldCard Entity',
            description: 'A test world-card for unit testing',
            status: 'draft',
            is_active: true,
      ...
      mockGetEntityEndpoint.mockReturnValue('/api/world_card')
  ...
  describe('Modify Test Entity Fields', () => {
    ...
    beforeEach(async () => {
      ...
      const mockGetEntityEndpoint = getEntityEndpoint as MockedFunction<typeof getEntityEndpoint>
      mockGetEntityEndpoint.mockReturnValue(`/api/world_card/${createdEntityId}`)
    })
    ...
    it('should modify name field', async () => {
      ...
      const mockResponse = {
        ...
        data: {
          ...
          data: {
            id: createdEntityId,
            name: 'Updated Test WorldCard Name',
            status: 'draft',
            is_active: true,
          }
        }
      ...
      const updateData = { name: 'Updated Test WorldCard Name' }
      const response = await authenticatedApi.patch(getEntityEndpoint(entityType, createdEntityId), updateData)

      expect(response.ok).toBe(true)
      expect(response.status).toBe(200)
      expect(response.data?.success).toBe(true)
      expect(response.data?.data.name).toBe('Updated Test WorldCard Name')
    })
    ...
    it('should modify description field', async () => {
      ...
      const mockResponse = {
        ...
        data: {
          ...
          data: {
            id: createdEntityId,
            description: 'Updated description for test world-card',
            status: 'draft',
            is_active: true,
          }
        }
      ...
      const updateData = { description: 'Updated description for test world-card' }
      const response = await authenticatedApi.patch(getEntityEndpoint(entityType, createdEntityId), updateData)

      expect(response.ok).toBe(true)
      expect(response.status).toBe(200)
      expect(response.data?.success).toBe(true)
      expect(response.data?.data.description).toBe('Updated description for test world-card')
    })
  ...
  describe('Create Fields in Spanish (es)', () => {
    it('should create world-card with Spanish fields', async () => {
      const { authenticatedApi, createTestData, getEntityEndpoint } = await import('../integration/helpers')
      const mockCreateTestData = createTestData as MockedFunction<typeof createTestData>
      const mockGetEntityEndpoint = getEntityEndpoint as MockedFunction<typeof getEntityEndpoint>
      const mockPost = authenticatedApi.post as MockedFunction<typeof authenticatedApi.post>
      ...
      const mockResponse = {
        ...
        data: {
          ...
          data: {
            id: 2,
            code: `${testCode}-es`,
            name: 'Carta Mundial de Prueba',
            description: 'Una carta mundial de prueba para pruebas unitarias',
            lang: 'es',
            status: 'draft',
            is_active: true,
          }
      ...
      expect(response.status).toBe(201)
      expect(response.data?.success).toBe(true)
      expect(response.data?.data.name).toBe('Carta Mundial de Prueba')
  ...
  describe('Delete Test Entity', () => {
    ...
    it('should delete the test world-card entity', async () => {
      const { authenticatedApi, getEntityEndpoint } = await import('../integration/helpers')
