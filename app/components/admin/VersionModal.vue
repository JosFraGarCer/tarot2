...
const emit = defineEmits<{ (e:'save', payload: { id?: number; version_semver: string; description?: string | null; metadata?: Record<string, any> }): void }>()

const isEdit = computed(() => Boolean(props.value && props.value.id))

const form = reactive<{ id?: number; version_semver: string; description?: string | null; metadata?: Record<string, any> | null; metadataText: string }>({
  id: props.value?.id,
